...
/*Implementation slider - Sticky scroll approach*/
    ...
    // Множник висоти для scroll (480vh для слайдів = 4.8)
    const SCROLL_HEIGHT_MULTIPLIER = 4.8;
    // Додатковий простір після останнього слайду (50vh = 0.5)
    const EXTRA_SPACE_AFTER_LAST = 0.5;
    // Загальна висота = 530vh (4.8 + 0.5 = 5.3)
    
    // Отримуємо реальну висоту контенту
    ...
    // Висота секції для scroll = viewport height * (множник + додатковий простір)
    // 480vh для слайдів + 50vh після останнього = 530vh загалом
    ...
    const step = (dir) => {
        ...
        // На мобільних просто перемикаємо items
        if (isMobile()) {
            setActiveIndex(newIndex);
            return;
        }
        
        // На десктопі - скролимо до правильного місця
        const vh = window.innerHeight || document.documentElement.clientHeight;
        const sectionTop = section.offsetTop;
        const scrollHeight = getScrollHeight();
        const contentHeight = getContentHeight();
        
        // Progress для нового item (0-1 для кожного item)
    ...
    // Обчислюємо progress скролу всередині секції (0-1)
    const getScrollProgress = () => {
        if (isMobile()) return 0; // На мобільних не використовуємо scroll progress
        
        const vh = window.innerHeight || document.documentElement.clientHeight;
        const scrollHeight = getScrollHeight();
        
        // Коли секція починається (top = 0) -> progress = 0
        // Коли секція закінчується (top = scrollHeight - vh) -> progress = 1
        const sectionTop = section.offsetTop;
    ...
    // Оновлюємо активний item на основі scroll progress
    const updateActiveItemFromScroll = () => {
        if (isMobile()) return; // На мобільних не оновлюємо через scroll
        
        const progress = getScrollProgress();
        
        // Обчислюємо item index з урахуванням того, що останній слайд має додатковий простір
        // Progress 0.0 -> item 1, progress ~0.906 (480/530) -> item 5, progress 1.0 -> item 5
        // Перші 4 слайди розподілені на 480vh, останній слайд + 50vh = 530vh
        ...
        if (progress >= slidesProgress) {
            // Останній слайд (5) активний від progress ~0.906 до 1.0
            itemIndex = MAX_ITEMS;
        } else {
            // Перші 4 слайди розподілені на progress 0.0 - ~0.906
            const normalizedProgress = progress / slidesProgress; // нормалізуємо до 0-1
            itemIndex = Math.floor(normalizedProgress * (MAX_ITEMS - 1)) + 1;
        }
        
        const clampedIndex = clamp(itemIndex, 1, MAX_ITEMS);
        
        // Оновлюємо тільки якщо змінився
    ...
    // -------------------------
    // SCROLL LISTENER: оновлення активного item
    // -------------------------
    ...
    updateActiveItemFromScroll(); // початковий стан
